<?php
session_start();
error_reporting(0);
include('dbconnection.php');
if ($_SESSION['stloggedin'] != 1) {
    header('location:index.php');
} else {
    $stid = $_SESSION['stID'];
    $ret = mysqli_query($con, "select * from stlogin where id='$stid'");
    $row = mysqli_fetch_array($ret);
    $roll = $row['rollno'];




?>


    <!DOCTYPE html>
    <html lang="en">

    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/css/bootstrap.min.css" integrity="sha384-zCbKRCUGaJDkqS1kPbPd7TveP5iyJE0EjAuZQTgFLD2ylzuqKfdKlfG/eSrtxUkn" crossorigin="anonymous">
        <link rel="stylesheet" href="style1.css">
        <link rel="stylesheet" href="style.css">
        <title>Specific Dates Entry</title>
    </head>

    <body>

        <!-- <div class="d-flex flex-row " style="height:90px;"> -->
        <div class="fluid-container">
            <div class="row">
                <div class="col-12 col-md-4">
                    <img src="images/logo.jpg" alt="IIITR logo" style="width:100%" />
                </div>
                <div class="col-md-4 col-12 d-flex flex-column justify-content-center">
                    <a href='search-page.php'><button class="button2 mt-3 mr-3 ml-3" style="background-color:black; color:white;width: 100%; ">Search Entries</button></a>
                </div>
                <div class="col-md-4 col-12">
                    <div class="d-flex flex-row ml-3">
                        <img src="data:image/png;base64," class="login-logo" />

                        <div class="d-flex flex-column justify-content-center ">

                            <a href='logout.php'>
                                <button class="logout" name="logout" style="background-color:black; color:white;  width:90px;  height:30px;    border-radius: 5px;    margin-bottom:10px; margin-left:12px; margin-top:0px;" href='logout.php'>LOG OUT</button>
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="statbox">
            <div class="fluid-container" style="background-color:white;">
                <div class="row">
                    <div class="col-12 col-lg-4 left-bar pt-3" style="background-color:white;">
                        <a href="sthomepage.php"><button class="button2 button" style="width:80%;">Home Page</button></a><br>
                        <a href="sthostel.php"><button class="button2 button" style="width:80%;">Hostel</button></a><br>
                        <a href="stacadblockspdate.php"><button class="button2 button" style="width:80%;">Academic Block</button></a><br>
                        <a href="stsportsspecdate.php"><button class="button2 button" style="width:80%;">Sports records</button></a><br>
                        <a href="stlibraryspecdate.php"><button class="button2 button" style="width:80%;">Library</button></a><br>
                    </div>
                    <?php
                    $query1 = mysqli_query($con, "select * from hostelentry where rollno='$roll' order by date");
                    $count_hostelentries = mysqli_num_rows($query1);
                    $query2 = mysqli_query($con, "select * from acadblockentry where rollno='$roll' order by date");
                    $count_acadentries = mysqli_num_rows($query2);
                    $query3 = mysqli_query($con, "select * from sportsentry where rollno='$roll' order by date");
                    $count_sportsentries = mysqli_num_rows($query3);
                    $query4 = mysqli_query($con, "select * from libraryentry where rollno='$roll' order by issuedate");
                    $count_libraryentries = mysqli_num_rows($query4);
                    ?>
                    <div class="col-12 col-md-6 col-lg-4">
                        <div class="visitor-card text-center card1" style="background-color:#ffc252;">
                            <h4>Hostel entries till now</h4>
                            <h1 class="count"><?php echo $count_hostelentries ?></h1>
                        </div>
                        <div class="visitor-card text-center" style="background-color:#ffc252;">
                            <h4>sports entries till now</h4>
                            <h1 class="count"><?php echo $count_sportsentries ?></h1>
                        </div>

                    </div>
                    <div class="col-12 col-md-6 col-lg-4">
                        <div class="visitor-card text-center card1" style="background-color:#ffc252;">
                            <h4>Acad block entries till now</h4>
                            <h1 class="count"><?php echo $count_acadentries ?></h1>
                        </div>
                        <div class="visitor-card text-center" style="background-color:#ffc252;">
                            <h4>Library entries till now</h4>
                            <h1 class="count"><?php echo $count_libraryentries ?></h1>
                        </div>
                    </div>
                </div>
            </div>
        
        <div class="text-center copyright-section">
            <span style="color:red;">&#169</span> <span style="font-style:italic;"><B> MANAGING MASTERS 2022.All rights reserved</B></span>
        </div>


    </body>

    </html>
<?php } ?>