<?php
session_start();
error_reporting(0);
include('dbconnection.php');
if ($_SESSION['stloggedin'] != 1) {
    header('location:index.php');
} else {
    $stid = $_SESSION['stID'];
    $ret = mysqli_query($con, "select * from stlogin where id='$stid'");
    $row = mysqli_fetch_array($ret);
    $roll = $row['rollno'];
    $name = $row['Name'];


?>
    <!DOCTYPE html>
    <html lang="en">

    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/css/bootstrap.min.css" integrity="sha384-zCbKRCUGaJDkqS1kPbPd7TveP5iyJE0EjAuZQTgFLD2ylzuqKfdKlfG/eSrtxUkn" crossorigin="anonymous">
        <link rel="stylesheet" href="style.css">
        <title>Student Home Page</title>
    </head>

    <body>
        <!-- <div style="background-color:white;">
            <div class="d-flex flex-row " style="height:55px;"> -->
        <div class="fluid-container">
            <div class="row">
                <div class="col-md-6 col-12">
                    <img src="images/logo.jpg" alt="IIITR logo" style="width:100%" class="img1" />
                </div>
                <div class="col-md-6 col-12 d-flex mr-auto">

                    <div class="d-flex flex-row ">
                        <!-- <img src="data:image/png;base64," class="login-logo"> -->
                        <div>
                            <h5 class="" style="text-decoration:underline; font-size:21px; font-weight:bold; margin-left:500px;">
                                <?php echo $name ?>
                            </h5>
                            <a href="logout.php" transition:background-color:white 400ms linear ease-in;>
                                <button class="logout" name="logout" style="background-color:black;color:white;width:100px;height:40px;border-radius:5px;margin-bottom:10px; margin-left:500px;  margin-top:0px;" href='logout.php'>LOG OUT</button>
                            </a>
                        </div>
                    </div>

                </div>
            </div>
        </div>
        <div style="background-color:white;">
            <div class="fluid-container">
                <div class="row">
                    <div class="col-12 col-lg-4 side-bar" style="background-color:white; margin-top:11px;">
                        <a href="view_studentdetails.php"><button class="button2" style="width:80%;">View Entries till now</button></a><br>
                        <a href="request.php"><button class="button2" style="width:80%;">Request Change</button></a><br>
                        <a href="st_specificdateentry.php"><button class="button2" style="width:80%;">Entries between specific dates</button></a><br>
                        <a href="stviewrequest.php"><button class="button2" style="width:80%;">Your Request</button></a><br>
                        <a href="st_changepassword.php"><button class="button2" style="width:80%;">Change Password</button></a><br>
                    </div>
                    <?php
                    $query1 = mysqli_query($con, "select * from hostelentry where rollno='$roll' order by date");
                    $count_hostelentries = mysqli_num_rows($query1);
                    $query2 = mysqli_query($con, "select * from acadblockentry where rollno='$roll' order by date");
                    $count_acadentries = mysqli_num_rows($query2);
                    $query3 = mysqli_query($con, "select * from sportsentry where rollno='$roll' order by date");
                    $count_sportsentries = mysqli_num_rows($query3);
                    $query4 = mysqli_query($con, "select * from libraryentry where rollno='$roll' order by issuedate");
                    $count_libraryentries = mysqli_num_rows($query4);
                    ?>
                    <div class="col-12 col-md-6 col-lg-4" style="background-color: white;">

                        <div class="visitor-card text-center card1" style="background-color:aqua; ">
                            <h4>Hostel entries</h4>
                            <h1 class="count"><?php echo $count_hostelentries ?></h1>
                        </div>
                        <div class="visitor-card text-center" style="background-color: aqua;  ">
                            <h4>Sports entries</h4>
                            <h1 class="count"><?php echo $count_sportsentries ?></h1>
                        </div>

                    </div>
                    <div class="col-12 col-md-6 col-lg-4" style="background-color: white;">
                        <div class="visitor-card text-center card1" style="background-color:aqua;  ">
                            <h4>Acad block entries</h4>
                            <h1 class="count"><?php echo $count_acadentries ?></h1>
                        </div>

                        <div class="visitor-card text-center" style="background-color:aqua;  ">
                            <h4>Library entries</h4>
                            <h1 class="count"><?php echo $count_libraryentries ?></h1>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        </div>
        <div class="text-center copyright-section" style="margin-top:20px;">
            <span style="color:red;">&#169</span> <span style="font-style:italic;"><b> MANAGING MASTERS 2022.All rights reserved</b></span>
        </div>


    </body>

    </html>
<?php } ?>